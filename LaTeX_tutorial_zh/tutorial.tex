...
\begin{figure}[!hbt]
    ...
    \subcaptionbox
        ...
        {\includegraphics[width=0.3\linewidth]{logo-Debian.pdf}}
    \vspace{\baselineskip} % 分隔上下列
    \subcaptionbox
        {GNU (A project of OS)
...
main.tex中再以\textbackslash{}input\{filename\}引入其他tex檔。除了\textbackslash{}input\{\}外，亦可用\textbackslash{}include\{\}。兩者差異在於\textbackslash{}include\{\}一定會另起一頁，且雙面配置時一定由右頁開始
...
設定。不過這指令只能放在standalone class （子檔）中，所以另外加上\textbackslash{}IfStandalone來判斷是否為子檔。這裡放的``float=true''指的是單獨編譯子檔時是否啟用浮動環境（圖、表等等），預設是不啟用的。
...
\section{字型}
\label{sec:s_fonts}
電腦上常用的中文字型有四大基本類型，分別為楷、明、宋、黑（這是不包含行書之類的非正式文書字體）。楷體較粗，明、宋較細。而黑體則是各筆劃粗細相同（常用於廣告？）。一般來說文件的內文應該要是較細的文字（如明、宋體），避免閱讀疲勞。因此此樣板設定封面、標題為楷體（英文則是預設無襯線字），其餘皆為明體。
...
此樣板可以在config.tex中分別設定中文以及英文字型。預設封面為楷體中文與無襯線英文，章節標題亦為楷體中文與無襯線英文，其餘內文均為明體中文與襯線英文。\cref{fig:titleFontStyle}為預設風格與自訂的一個示範。
\begin{figure}
    \centering
    \subcaptionbox
        {預設值，楷體中文+無襯線英文
...
``>'' 指的是在欄位之前執行，``<'' 指的是在欄位之後執行。以直接讓欄位進入數學模式為例，欄位屬性應該寫成：\verb|{>{$}c<{$}}| 也就是各行(column)置中，並且前後均以\$包起來。
...
其他還有很多選項，一般依照預設值就可以了。不過有一個選項``draft''很有用。它會略去圖形、連結的處理。所以編譯速度會較快(以這份教學為例，draft約減少20\%時間)。要啟用的話只要將它加入方括號中即可。本樣板採用 standalone 處理子文件，這裡分別示範主文件、子文件添加選項方法。\\
主文件：
\begin{lstlisting}
...
其他的編輯器同理，請自行找尋如何載入.cwl。對於自訂指令也可以自寫.cwl來解決。本樣板也提供sppmgTools.cwl給我自製的指令使用。
...
\footnote{同理，NCU圖書館查書可以用\url{http://opac.lib.ncu.edu.tw/search*cht/a?searchtype=Y&searcharg=\%S}
    以及關鍵字``lib''來處理。}
...
首先示範使用\LaTeX\ 預設值（\textbackslash{}titleStyle\{0\}）的情況，如\cref{fig:titleStyle_0}：
\begin{figure}[!h]
    \centering
    \subcaptionbox
        {config.tex 中的設定}
